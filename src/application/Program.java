package application;

import entities.Product;

import java.io.*;
import java.text.ParseException;
import java.util.ArrayList;
import java.util.List;
import java.util.Locale;
import java.util.Scanner;

public class Program {

    public static void main(String[] args) throws ParseException {
        Locale.setDefault(Locale.US);
        Scanner sc = new Scanner(System.in);

        // Lista dos produtos
        List<Product> list = new ArrayList<>();

        System.out.println("Enter file path: ");
        String sourceFileStr = sc.nextLine();

        // Passar o caminho do file
        File sourceFile = new File(sourceFileStr);
        // getParent obter o caminho despresando o nome do arquivo
        String soucefolderStr = sourceFile.getParent();

        System.out.println(soucefolderStr);

        // Criar a pasta out  - mkdir cria novo diretorio
        boolean success = new File(soucefolderStr + "\\out").mkdir();
       //  System.out.println("Folder created: "+ success);

        // Criar o arquivo de saida
        String targetFileStr = soucefolderStr + ("\\out\\summary.csv");

        try (BufferedReader br = new BufferedReader(new FileReader(sourceFileStr))){

            String itemCsv = br.readLine();
            while (itemCsv != null){
                //Leu a primeira linha, imprimiu e passou para a próxima
              //  System.out.println(itemCsv);

                String[] fields = itemCsv.split(",");
                String name = fields[0];
                double price = Double.parseDouble(fields[1]);
                int quantity = Integer.parseInt(fields[2]);

                list.add(new Product(name, price, quantity));
                itemCsv = br.readLine();
            }

            try(BufferedWriter bw = new BufferedWriter(new FileWriter(targetFileStr))){

                for (Product item: list){
                    bw.write(item.getName() + "," +String.format("%.2f" , item.total()));
                    bw.newLine();
                }
                System.out.println(targetFileStr + " CREATED");
            }catch (IOException e){
                System.out.println("Error writing file: " + e.getMessage());
            }

        }catch (IOException e){
            System.out.println("Error writing file: " + e.getMessage());
        }
        sc.close();
    }
}
